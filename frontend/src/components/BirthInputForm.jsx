...
function BirthInputForm({ title, storageKey, onDataChange }) {
  ...
  const handleBirthTimeChange = useCallback((value) => {
  ...
  return (
    <div className="bg-slate-800/50 backdrop-blur-sm rounded-lg p-4 sm:p-6 shadow-xl border border-slate-700" 
      ...
      <div className="space-y-3 sm:space-y-4" style={{ overflow: 'visible', position: 'relative', zIndex: 1 }}>
        {/* 생년월일 */}
        <div>
          <label htmlFor={`birthDate-${storageKey}`} 
          ...
          <IMaskInput
            ...
            className="w-full px-3 sm:px-4 py-2.5 sm:py-2 text-sm sm:text-base bg-slate-700 border border-slate-600 rounded-lg text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent touch-manipulation"
          />
        </div>

        {/* 태어난 시간 */}
        <div>
          <label htmlFor={`birthTime-${storageKey}`} 
                 className="block text-xs sm:text-sm font-medium text-slate-300 mb-1.5 sm:mb-2">
            태어난 시간
          </label>
          <IMaskInput
          ...
          <div className="flex gap-4">
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                className="w-4 h-4 text-blue-500 bg-slate-700 border-slate-600 focus:ring-2 focus:ring-blue-500 focus:ring-offset-0 focus:ring-offset-slate-800"
              />
              <span className="text-sm sm:text-base text-slate-300">남자</span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                type="radio"
                ...
                className="w-4 h-4 text-blue-500 bg-slate-700 border-slate-600 focus:ring-2 focus:ring-blue-500 focus:ring-offset-0 focus:ring-offset-slate-800"
              />
              <span className="text-sm sm:text-base text-slate-300">여자</span>
            </label>
          </div>
        </div>
        ...
        <div style={{ position: 'relative', zIndex: 10002 }}>
          <label htmlFor={`cityInput-${storageKey}`} 
                 className="block text-xs sm:text-sm font-medium text-slate-300 mb-1.5 sm:mb-2">
            태어난 도시
          </label>
          ...
          {birthData.cityData.name && (
            <p className="mt-2 text-xs text-slate-400 break-words">
              ...
              {birthData.cityData.timezone && ` (${birthData.cityData.timezone})`}
            </p>
          )}
        </div>
      </div>

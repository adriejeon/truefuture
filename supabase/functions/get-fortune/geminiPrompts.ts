...
## 운과 주의점
- **Good:** (해당 날짜에 하면 무조건 이득인 활동 3가지를 **최소 2문장씩** 구체적으로 나열하세요. '왜' 이득인지 점성학적 흐름을 현실적인 이유로 풀어 설명하세요. 예를 들어, 체육관에서 '천국의 계단' 같은 특정 운동 기구를 타며 땀을 흘리는 것이 길한지 등 구체적인 일상 행동을 제안해도 좋습니다.)
- **Bad:** (해당 날짜에 절대 피해야 할 행동 3가지를 **최소 2문장씩** 구체적으로 나열하세요. 무심코 했을 때 어떤 후폭풍이 오는지 경고하세요.)

## 💡 Real Tip
(해당 날짜에 직면할 수 있는 구체적 문제 상황에 대한 '명쾌한 행동 솔루션'을 5~8줄 분량으로 넉넉하게 제시하세요. 해당 날짜의 행운 컬러, 귀인이 될 사람의 특징, 스트레스 해소법(예: K3 드라이브 가기, 영어/파이썬 공부로 머리 식히기 등 다양한 라이프스타일에 맞춘 예시) 등을 유쾌하고 풍성하게 덧붙이세요.)
`;
}

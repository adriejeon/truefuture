...
2.  **형식 및 포맷 제약 (프론트엔드 파싱 오류 방지):**
    * **마크다운 볼드체(**) 기호 사용 절대 금지:** 문장 중간이나 단어 양옆에 강조를 위한 별표(**)를 절대 쓰지 마세요. 강조하고 싶다면 느낌표(!)를 쓰거나 문장의 어조를 단호하게 바꾸세요.
...
## 운과 주의점
- **Good:** (해당 날짜에 하면 무조건 이득인 활동 3가지를 **최소 2문장씩** 구체적으로 나열하세요. '왜' 이득인지 점성학적 흐름을 현실적인 이유로 풀어 설명하세요.)
- **Bad:** (해당 날짜에 절대 피해야 할 행동 3가지를 **최소 2문장씩** 구체적으로 나열하세요. 무심코 했을 때 어떤 후폭풍이 오는지 경고하세요.)

## 💡 Real Tip
(해당 날짜에 직면할 수 있는 구체적 문제 상황에 대한 '명쾌한 행동 솔루션'을 5~8줄 분량으로 넉넉하게 제시하세요. 해당 날짜의 행운 컬러, 귀인이 될 사람의 특징, 스트레스 해소법 등을 유쾌하고 풍성하게 덧붙이세요.)
`;
}
...
- ### YYYY년 M월 <- 이 부분은 텍스트 양옆에 별표(**) 기호를 절대 붙이지 마세요.

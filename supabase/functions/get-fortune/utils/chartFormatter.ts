...
function analyzeOuterPlanetAspects(
  ...
  for (const outerKey of outerPlanets) {
    ...
    if (natalAspects.length > 0 || transitAspects.length > 0) {
      let section = `\n${outerName} (${keyword}):\n`;
      section += `  위치: ${outerPlanetData.sign} ${outerPlanetData.degreeInSign.toFixed(1)}° (${formatPlanetHouseWsQs(outerPlanetData)})\n`;
...
/** 고전 룰러 표기명 → chartData.planets 키 (달 디스포지터 교차용) */
const CLASSICAL_RULER_TO_PLANET_KEY: Record<
  string,
  keyof ChartData["planets"]
> = {
  Sun: "sun",
  Moon: "moon",
  Mercury: "mercury",
...
/**
 * Gemini용 행성/하우스 줄 예시 (아래 헬퍼가 동일 패턴으로 생성):
 *   - MOON: Taurus 15.0° (WS: 8H / QS: 7H - Angle)
 *   - VENUS: Virgo 3.0° (WS: 12H / QS: 12H - Cadent)
 * 궁합 Moon 디스포지터 교차 예시:
 *   User1 (내담자님)'s Moon is ruled by Venus. In User2 (상대방)'s chart, this Venus is located at (WS: 7H / QS: 7H - Angle).
 */
function hasQsFields(p: PlanetHouseQs): p is PlanetHouseQs & {
  qsHouse: number;
  qsStrength: QuadrantStrength;
} {
  return (
    typeof p?.qsHouse === "number" &&
    p.qsStrength != null &&
    (p.qsStrength === "Angle" ||
      p.qsStrength === "Succedent" ||
      p.qsStrength === "Cadent")
  );
}

/**
 * 네이탈/SR 행성 줄 괄호 안: Whole Sign + 알카비티우스 QS.
 * 다른 모듈(geminiPrompts 등)에서도 사용 — 구버전 차트는 QS 생략 시 WS만.
 */
export function formatPlanetHouseWsQs(
  planet: PlanetHouseQs | null | undefined,
  opts?: { sr?: boolean },
): string {
  const ws = planet?.house;
  if (planet == null || typeof ws !== "number") {
    return opts?.sr ? "SR WS: —" : "WS: —";
  }
  if (!hasQsFields(planet)) {
...
function formatNatalQsReferenceBlock(natalData: ChartData): string {
  ...
  return `
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
[Natal 참고 — 발광체·포르투나 (WS / Alcabitius QS)]
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
${lines.join("\n")}
...
function formatMoonLordInPartnerChartLineForLocale(
  moonOwnerLabel: string,
  partnerLabel: string,
  moonOwnerChart: ChartData,
  partnerChart: ChartData,
  locale: "ko" | "en",
): string {
  const moonSign = moonOwnerChart.planets?.moon?.sign;
  if (!moonSign) {
    return locale === "ko"
      ? `  - (${moonOwnerLabel} 달 정보 없음)`
      : `  - (No Moon data for ${moonOwnerLabel})`;
  }
  const lord = getSignRuler(moonSign);
  const key = CLASSICAL_RULER_TO_PLANET_KEY[lord];
  const lordBody = key ? partnerChart.planets?.[key] : undefined;
  const at = lordBody ? formatPlanetHouseWsQs(lordBody) : "";
  if (!lordBody || lord === "Unknown") {
    return locale === "ko"
      ? `  - ${moonOwnerLabel}의 달은 ${lord}의 지배를 받습니다. ${partnerLabel} 차트에서 해당 행성 데이터 없음`
      : `  - ${moonOwnerLabel}'s Moon is ruled by ${lord}. In ${partnerLabel}'s chart, data for this planet is missing.`;
  }
  return locale === "ko"
    ? `  - ${moonOwnerLabel}의 달은 ${lord}의 지배를 받습니다. ${partnerLabel} 차트에서 이 ${lord}는 (${at})에 있습니다.`
    : `  - ${moonOwnerLabel}'s Moon is ruled by ${lord}. In ${partnerLabel}'s chart, this ${lord} is located at (${at}).`;
}

/** 궁합: 달의 고전 디스포지터가 상대 네이탈의 WS/QS에 놓인 위치 (한·영 공용). */
export function buildSynastryMoonDispositorSection(
  ...
  const l2 = formatMoonLordInPartnerChartLineForLocale(
    u2,
    u1,
    natalData2,
    natalData1,
  ...
  const title =
    locale === "ko"
      ? "[Synastry — 상대 차트 속 달의 디스포지터 (WS / Alcabitius QS)]"
      : "[Synastry — Moon Dispositor in Partner Chart (WS / Alcabitius QS)]";
  const hint =
    locale === "ko"
      ? "한쪽 달의 별자리 고전 디스포지터가 상대 네이탈에서 Whole Sign·QS(힘)상 어디에 있는지입니다. 정서 교차·수용 해석에 반영하세요."
      : "Classical dispositor of each person's Moon sign, placed in the partner's natal chart with Whole Sign house and Alcabitius Quadrant Strength. Use for emotional crossover and receptivity.";
  return `
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
${title}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
/** 데일리: 트랜짓 달·연주 행성이 네이탈 WS/QS 어디를 지나는지 (한글). */
function formatDailyTransitNatalOverlayKo(
  ...
  if (moonPl) {
    lines.push(
      `트랜짓 달(Transit Moon)이 회원님 네이탈 기준 WS ${moonPl.wsHouse}H / QS ${moonPl.qsHouse}H (${moonPl.qsStrength}) 구간을 통과합니다.`,
    );
  } else if (fallbackWsMoon != null) {
    lines.push(
      `트랜짓 달은 네이탈 Whole Sign ${fallbackWsMoon}하우스 참고 (QS 상세는 배치 데이터 없음).`,
    );
  }
  if (lordName && lordPl) {
    lines.push(
      `트랜짓 연주 행성 Transit ${lordName}은(는) 회원님 네이탈 기준 WS ${lordPl.wsHouse}H / QS ${lordPl.qsHouse}H (${lordPl.qsStrength})에 있습니다.`,
    );
  }
  return lines.join("\n");
}
 ...
 * - 리셉션/리젝션 메타 태그로 길흉 가이드.
 * 출력 순서: 1. 내담자·프로펙션 요약 1b. 네이탈 Sun/Moon/POF (WS·QS) 2. 오전 흐름 3. 오후 흐름 4. 4대 감응점 타격 경보 5. 리셉션/리젝션
 */
...
${formatDailyTransitNatalOverlayKo(
...
export function formatSolarReturnBlockForPrompt(
  ...
  const srPlanets = Object.entries(srChartData.planets)
    .map(([name, planet]) => {
      return `  - ${name.toUpperCase()}: ${planet.sign} ${planet.degreeInSign.toFixed(1)}° (${formatPlanetHouseWsQs(planet, { sr: true })})`;
    })
    .join("\n");
...
export function generateYearlyUserPrompt(
  ...
  const natalPlanets = Object.entries(natalData.planets)
    .map(([name, planet]) => {
      return `  - ${name.toUpperCase()}: ${
        planet.sign
      } ${planet.degreeInSign.toFixed(1)}° (${formatPlanetHouseWsQs(planet)})`;
    })
    .join("\n");
  ...
  const solarReturnPlanets = Object.entries(solarReturnData.planets)
    .map(([name, planet]) => {
      return `  - ${name.toUpperCase()}: ${
        planet.sign
      } ${planet.degreeInSign.toFixed(1)}° (${formatPlanetHouseWsQs(planet, { sr: true })})`;
    })
    .join("\n");
...
`
  ...
  const moonLordQsSection = buildSynastryMoonDispositorSection(
    natalData1,
    natalData2,
    "ko",
  );
 ...
 * @param graphKnowledge - 차트 위계/섹트 기반 점성학 지식 (선택 또는 빈 문자열 허용)
...
- 7th House Ruler: ${seventhRuler}`);
  ...
  if (consultationTransitChart?.planets) {
    const transitPlanetLines = Object.entries(consultationTransitChart.planets)
      ...
      .join("\n");
    sections.push(`[🌍 Transit Chart — 상담 시점 하늘 (WS / Alcabitius QS)]
${transitPlanetLines}
각 행성의 QS는 시스템 프롬프트의 Quadrant Strength 해석 규칙으로 가중치를 두세요.`);
  }
...
${careerAnalysis.candidates
  .map((c) => {
    ...
    const pKey = c.planetName.toLowerCase() as keyof ChartData["planets"];
    const pData = chartData.planets?.[pKey];
    const placement = pData
      ? formatPlanetHouseWsQs(pData)
      : `WS: ${c.house}H (Whole Sign)`;
    return `  * ${c.planetName}: ${c.score} points. (Placement: ${placement}, ${c.sign})
...
- Meaning: "직업적으로 가장 강력한 행성은 ${
  ...
  if (wealthAnalysis) {
    const occList =
      wealthAnalysis.occupants.length > 0
        ? wealthAnalysis.occupants
            .map((o) => {
              const pKey = o.planetName.toLowerCase() as keyof ChartData["planets"];
              const pData = chartData.planets?.[pKey];
              const qsBit = pData ? formatPlanetHouseWsQs(pData) : "";
              const qsSuffix = qsBit ? ` / ${qsBit}` : "";
              return `${o.planetName} (${o.type}; ${
                o.type === "Benefic"
    ...
    const rulerBody =
      chartData.planets?.[
        rulerPlanetKey as keyof ChartData["planets"]
      ];
    const rulerWsQsFrag = rulerBody
      ? `\n  * Ruler placement (WS/QS): ${formatPlanetHouseWsQs(rulerBody)}`
      : "";
...
- Ruler Condition: ${wealthAnalysis.ruler.planetName} (Score: ${
  ...
  * Status: ${rulerStatusText}${rulerSignCharacter ? `\n- Ruler Sign Character: "${rulerSignCharacter}"` : ""}${rulerWsQsFrag}
...
1. Venus Condition (Love Style):
   - Score: ${
     ...
     venus ? formatPlanetHouseWsQs(venus) : `${ord} House (WS only)`
   } (${sign})
...
Instruction: 제미나이에게 "이 데이터를 바탕으로 내담자의 연애 스타일, 미래 배우자의 특징(성격/직업), 그리고 결혼이 유력한 시기를 구체적으로 서술하라"고 지시해.`);
  ...
  if (
    ...
    const moonWsQs = moonPlanet
      ? formatPlanetHouseWsQs(moonPlanet)
      : "—";
    const moonIssueList =
...
- 내담자의 질문에 맞춰 현재 건강 상태, 회복 시기, 치료 방법, 정신 건강 관리를 구체적으로 조언하세요.`);
  ...
  sections.push(`[📚 Knowledge Base (차트 위계/섹트/헤이즈)]
...
/** chartData.planets에서 Sun, Moon, Venus, Mars, Jupiter, Saturn을 "Sun: Scorpio (WS: 11H / QS: 10H - Succedent)" 형식으로. getSignCharacter 주입 시 Sun/Moon에 (Character: ...) 추가 */
